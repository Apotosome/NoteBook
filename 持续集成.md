...
```bash
[root @ dev ~]  rpm -ivh jdk-16.0.1_linux-x64_bin.rpm 
...
2. 安装Maven
   ...
   ```bash
   [root @ jenkins ~]  tar xvf apache-maven-3.8.1-bin.tar.gz 
   [root @ jenkins ~]  mv apache-maven-3.8.1  /usr/local/
   [root @ jenkins ~]  cd /usr/local/
   [root @ jenkins /usr/local]  ln -s apache-maven-3.8.1/ maven		# 软链接成maven
...
1. 备份Maven全局配置文件
   ...
   ```bash
   [root @ jenkins /usr/local/maven]  cd /usr/local/maven/conf/
   [root @ jenkins /usr/local/maven/conf]  cp settings.xml{,.bak}		# 备份全局配置文件
   [root @ jenkins /usr/local/maven/conf]  ll
...
配置仓库两个选项
1、项目下的pom.xm1配置、只生效当前的项目 2、在 maven配置全局所有项目生效
